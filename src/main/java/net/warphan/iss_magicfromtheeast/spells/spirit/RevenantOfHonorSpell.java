...
public class RevenantOfHonorSpell extends AbstractSpell {
    private final ResourceLocation spellID = ResourceLocation.fromNamespaceAndPath(ISS_MagicFromTheEast.MOD_ID, "revenant_of_honor");

    @Override
    public List<MutableComponent> getUniqueInfo(int spellLevel, LivingEntity caster) {
        return List.of(
                Component.translatable("ui.irons_spellbooks.hp", getSamuraiHealth(spellLevel, null)),
                Component.translatable("ui.irons_spellbooks.damage", getSamuraiDamage(spellLevel, null)),
    ...
    public void onCast(Level world, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        ...
        if (!recast.hasRecastForSpell(this)) {
            ...
            world.addFreshEntity(spiritSamurai);

            spiritSamurai.finalizeSpawn((ServerLevel) world, world.getCurrentDifficultyAt(spiritSamurai.getOnPos()), MobSpawnType.MOB_SUMMONED, null, null);

            SummonManager.initSummon(entity, spiritSamurai, summonTime, summonedEntitiesCastData);
